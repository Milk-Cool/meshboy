...
void loop() {
    ...
    if(sio_hw->gpio_in & P(GB_RD)) {
        ...
        if(addr >= 0xa100 && addr < 0xa200) uart_tx_buf[addr - 0xa100] = data; // set data to write to esp32
        else if(addr == 0xa200) uart_n = Serial.readBytes(uart_rx_buf, data); // read data from esp32
        else if(addr == 0xa201) Serial.write(uart_tx_buf, data); // send data to esp32
    } else {
        // read operation
        sio_hw->gpio_oe_set |= GB_D;

        addr = (sio_hw->gpio_in & (P(GB_A_MAX + 1) - 1)) >> GB_A_MIN;
        if(addr >= 0xa000 && addr < 0xa100) data = uart_rx_buf[addr - 0xa000]; // reading from read data buffer
        else if(addr == 0xa202) data = uart_n; // read data byte count
