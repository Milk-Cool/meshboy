...
uint8_t ram[0x2000]; // 8KB; could have gotten away with just having the above variable in this one... but it's whatever really
...
void loop() {
    ...
    if(sio_hw->gpio_in & P(GB_RD)) {
        ...
        else if(addr >= 0xa000 && addr < 0xc000) ram[addr - 0xa000] = data; // writing to ram
    } else {
        // read operation
        ...
        else if(addr >= 0xa000 && addr < 0xc000) data = ram[addr - 0xa000]; // reading from ram
